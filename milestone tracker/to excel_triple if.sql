...
/*FPKick_real is for real Kick-off meeting time instead of predict one*/
  FPKick_real_Due as(
    SELECT
    [ProjectSizeId]
    ,CAST([Current_Date]as date)as FP_realKick_Due
    ,RANK() OVER (PARTITION by ProjectSizeId,PROJECT_SIZE_ID,PROJECT_SIZE_NAME ORDER by [Current_Date] ASC) as FPrnk_Due
...
FPKick_real_End as(
  ...
  ,CAST([EndDate]as date)as FP_realKick_End
  ...
  where Category_Name='Factory Prototype Kick'
  AND Item_Name='Kick-off meeting'
  AND [T_PID_CkList].Frequency=1
),
  FPKick_cp as(
    SELECT [T_PID_CkList].[PROJECT_SIZE_ID]
      ,[Category_Name]
...
SELECT [ProjectSizeId]
  ...
  PRKick_real_Due as(
    SELECT
    [ProjectSizeId]
    ,CAST([Current_Date]as date)as PR_realKick_Due
  ,RANK() OVER (PARTITION by ProjectSizeId,PROJECT_SIZE_ID,PROJECT_SIZE_NAME ORDER by [Current_Date] ASC) as PRrnk_Due
  FROM [PR_WEB2].[dbo].[T_GT_ProjectSchedule]
  left join [PR_WEB2].[dbo].[T_GT_TaskBaseInfo]
  on [T_GT_ProjectSchedule].[TASK_ID]=[T_GT_TaskBaseInfo].[TASK_ID]
  left join [PR_WEB2].[dbo].[T_PS_ProdSizeRelateInfo]
  ...
  PRKick_real_End as(
    SELECT [T_PID_CkList].[PROJECT_SIZE_ID]
    ,CAST([EndDate]as date)as PR_realKick_End
    FROM [PR_WEB2].[dbo].[T_PID_CkList]
    LEFT JOIN [T_PID_CkListFiles]
    ON ([T_PID_CkListFiles].[PROJECT_SIZE_ID] = [T_PID_CkList].[PROJECT_SIZE_ID]
    and [T_PID_CkListFiles].[MpType_ID] = [T_PID_CkList].[MpType_ID]
    ...
    and [T_PID_CkListFiles].[Frequency] = [T_PID_CkList].[Frequency]
    /* Frequency is duplicated phase count (e.g. Twice FP) */)
    where Category_Name='Pilot Run Kick'
    AND Item_Name='Kick-off meeting'
    AND [T_PID_CkList].Frequency=1
  ),
  PRKick_cp as(
...
SELECT [ProjectId]
  ...
  CASE       
       WHEN [FP_realKick_End] IS NULL AND [FP_realKick_Due] IS NULL THEN [FP_CP_deadline]
       WHEN [FP_realKick_End] IS NULL THEN [FP_realKick_Due]
       WHEN [FP_realKick_End] IS not NULL then [FP_realKick_End]
      ELSE 'error'
  END AS FP_realKick_result,/* decide the upcoming status*/
  [FP_realKick_End],
  [FP_realKick_Due],
  [FP_CP_deadline],
  [FP_Status],
  ...
  CASE       
       WHEN [PR_realKick_End] IS NULL AND [PR_realKick_Due] IS NULL THEN [PR_CP_deadline]
       WHEN [PR_realKick_End] IS NULL THEN [PR_realKick_Due]
       WHEN [PR_realKick_End] IS not NULL then [PR_realKick_End]
      ELSE 'error'
  END AS PR_realKick_result,
  [PR_realKick_End],
  [PR_realKick_Due],
  [PR_CP_deadline],
  [PR_Status],
  ...
  JOIN FPKick_real_Due on FPKick_real_Due.ProjectSizeId = latest_mpdate.PROJECT_SIZE_ID
  JOIN FPKick_real_End on FPKick_real_End.PROJECT_SIZE_ID = latest_mpdate.PROJECT_SIZE_ID
  JOIN PRKick_real_Due on PRKick_real_Due.ProjectSizeId= latest_mpdate.PROJECT_SIZE_ID
  JOIN PRKick_real_End on PRKick_real_End.PROJECT_SIZE_ID= latest_mpdate.PROJECT_SIZE_ID
  WHERE MPrnk = 1 AND MRrnk2=1 AND FPrnk1=1 AND FPrnk2=1 AND PRrnk=1 AND PRrnk2=1 and PRrnk_Due=1 AND FPrnk_Due=1
  ...
  ORDER BY latest_mpdate.PROJECT_SIZE_ID